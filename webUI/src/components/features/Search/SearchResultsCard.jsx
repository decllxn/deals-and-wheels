...
const SearchResultsCard = ({ listing }) => {
  ...
  const imageUrl =
    images?.[0]?.image || "/placeholder-car.jpg";
  ...
  return (
    <motion.div
      ...
      className="group bg-[var(--surface-color)]/75 backdrop-blur-md 
    ...
    >
      ...
      >
        ...
        <div className="px-4 py-2 text-xs border-t border-[var(--border-color)]/40 text-[var(--muted-text)]">
          Listed on:{" "}
          {created_at ? new Date(created_at).toLocaleDateString() : "Unavailable"}
        </div>
      </Link>
    </motion.div>
  );
