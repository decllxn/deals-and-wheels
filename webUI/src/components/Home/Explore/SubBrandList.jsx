...
const SubBrandList = ({ subBrands }) => (
  <div>
    <h3 className="text-2xl font-semibold text-gray-800 mt-8 mb-8">Explore Sub-Brands & Tuning</h3>
    <div className="bg-gray-100 rounded-lg p-5 md:p-6">
      <details className="group">
        <summary className="flex items-center justify-between text-gray-800 font-medium cursor-pointer list-none">
          <span className="text-base md:text-lg">Sub-Brands & Tuning Companies</span>
        ...
        <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-5">
          {subBrands.sort((a, b) => a.name.localeCompare(b.name)).map((item, index) => (
            <Link
              to={`/cars/sub-brand/${item.name.toLowerCase().replace(/\s+/g, '-')}`}
              key={index}
            ...
            >
              <div className="w-auto h-auto max-w-[56px] max-h-[36px] mr-3 flex items-center justify-center">
                <img src={item.logoSrc} alt={item.name} className="max-w-full max-h-full object-contain" />
              </div>
              {item.name}
            </Link>
          ))}
