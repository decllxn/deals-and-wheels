...
import {
  Bold, Italic, Underline, Highlighter, Code, Heading1, Heading2, Heading3,
  List, ListOrdered, CheckSquare, AlignLeft, AlignCenter, AlignRight, AlignJustify,
  Code2, ImageIcon, LinkIcon, Youtube, Palette, Minus, Undo2, Redo2, Eraser, Upload
} from 'lucide-react';
...
const ToolbarButton = ({ onClick, icon, active, label }) => (
  <button
    onClick={onClick}
    title={label}
    className={`group p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 transition-colors duration-150 hover:bg-primary/10 ${
      active ? 'text-primary bg-primary/10 ring-1 ring-primary/30' : 'text-muted-foreground'
    }`}
  >
...
const EditorToolbar = ({ editor }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 p-3 rounded-xl bg-card/90 dark:bg-card-dark/90 backdrop-blur-xl shadow-lg border border-muted transition-all">
      {/* Text Styles */}
      ...
      <div ref={headingRef} className="relative">
        ...
        />
        {headingOpen && (
          <div className="absolute left-0 mt-2 z-20 bg-popover rounded-md shadow-lg border border-muted flex flex-col space-y-1 p-2">
            <ToolbarButton onClick={() => chain().toggleHeading({ level: 1 }).run()} icon={<Heading1 size={14} />} active={isActive('heading', { level: 1 })} label="Heading 1" />
            <ToolbarButton onClick={() => chain().toggleHeading({ level: 2 }).run()} icon={<Heading2 size={14} />} active={isActive('heading', { level: 2 })} label="Heading 2" />
            <ToolbarButton onClick={() => chain().toggleHeading({ level: 3 }).run()} icon={<Heading3 size={14} />} active={isActive('heading', { level: 3 })} label="Heading 3" />
          </div>
        )}
      </div>
      ...
      <ToolbarButton onClick={() => chain().toggleOrderedList().run()} icon={<ListOrdered size={16} />} active={isActive('orderedList')} label="Ordered List" />
      <ToolbarButton onClick={() => chain().toggleTaskList().run()} icon={<CheckSquare size={16} />} active={isActive('taskList')} label="Task List" />

      {/* Alignment */}
      <ToolbarButton onClick={() => chain().setTextAlign('left').run()} icon={<AlignLeft size={16} />} active={isActive({ textAlign: 'left' })} label="Align Left" />
      <ToolbarButton onClick={() => chain().setTextAlign('center').run()} icon={<AlignCenter size={16} />} active={isActive({ textAlign: 'center' })} label="Align Center" />
      <ToolbarButton onClick={() => chain().setTextAlign('right').run()} icon={<AlignRight size={16} />} active={isActive({ textAlign: 'right' })} label="Align Right" />
      ...
      <ToolbarButton onClick={() => chain().setHorizontalRule().run()} icon={<Minus size={16} />} label="Horizontal Rule" />
      ...
      {/* Local Upload */}
      <label title="Upload Local Image">
      ...
      <ToolbarButton
        onClick={() => {
          const color = prompt('Enter color (e.g. red, #f00)');
